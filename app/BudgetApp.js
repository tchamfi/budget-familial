...
export default function BudgetApp(){
  ...
  return(<><GlobalStyles/><div className="grain"/><div style={{minHeight:"100vh",background:"#faf9f7"}}>
    ...
    {/* HEADER */}
    <div style={{background:"#1a1a2e",padding:"0 36px",position:"sticky",top:0,zIndex:100}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",maxWidth:1320,margin:"0 auto",height:68}}>
    ...
    {/* NAV */}
    <div style={{background:"#fff",borderBottom:"1px solid #e8e4dd",position:"sticky",top:68,zIndex:99}}><div style={{display:"flex",gap:2,maxWidth:1320,margin:"0 auto",padding:"10px 36px",overflowX:"auto"}}>{tabs.map(([k,l])=><button key={k} className="btn-press" onClick={()=>setTab(k)} style={{padding:"8px 18px",border:"none",borderRadius:9,cursor:"pointer",fontSize:13,fontWeight:600,background:tab===k?"#1a1a2e":"transparent",color:tab===k?"#faf9f7":"#8a8578",whiteSpace:"nowrap",position:"relative"}}>{l}{k==="alertes"&&alertCount>0&&tab!==k&&<span style={{position:"absolute",top:-2,right:-2,width:8,height:8,borderRadius:"50%",background:"#b91c1c"}}/>}</button>)}</div></div>
...
function ChgRow({charge,l:initL,o:initO,onSave,onDelete,tdS,iS,bP,bG,bD}){const[ed,setEd]=useState(false);const[l,setL]=useState(initL);const[o,setO]=useState(initO);useEffect(()=>{setL(initL);setO(initO);},[initL,initO]);return(<tr className="row-hover" style={{background:ed?"#fdfbf5":"transparent"}}><td style={tdS}>{charge.fields?.description}</td><td style={tdS}><span style={{display:"inline-block",padding:"3px 10px",borderRadius:6,fontSize:10,fontWeight:700,letterSpacing:"0.04em",background:charge.fields?.compte==="Perso"?"#fef2f2":"#eff6ff",color:charge.fields?.compte==="Perso"?"#b91c1c":"#1e40af",textTransform:"uppercase"}}>{charge.fields?.compte||"Commun"}</span></td><td style={{...tdS,textAlign:"right"}}>{ed?<input style={iS} type="number" value={l} onChange={e=>setL(+e.target.value)}/>:<span style={{fontVariantNumeric:"tabular-nums"}}>{fmt(initL)}</span>}</td><td style={{...tdS,textAlign:"right"}}>{ed?<input style={iS} type="number" value={o} onChange={e=>setO(+e.target.value)}/>:<span style={{fontVariantNumeric:"tabular-nums"}}>{fmt(initO)}</span>}</td><td style={{...tdS,textAlign:"right",fontWeight:700}}>{fmt((ed?l:initL)+(ed?o:initO))}</td><td style={tdS}><div style={{display:"flex",gap:6,justifyContent:"flex-end"}}>{ed?<><button className="btn-press" style={bP} onClick={()=>{onSave(l,o);setEd(false);}}>OK</button><button className="btn-press" style={bG} onClick={()=>{setL(initL);setO(initO);setEd(false);}}>X</button></>:<><button className="btn-press" style={{...bG,padding:"6px 12px",fontSize:11}} onClick={()=>setEd(true)}>Modifier</button><button className="btn-press" style={bD} onClick={onDelete}>Suppr.</button></>}</div></td></tr>);}
