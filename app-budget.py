...
# TABS
# ========================================
tabs = st.tabs(["Dashboard", "Revenus", "Charges", "Épargne", "Paramètres"])
...
with tabs[2]:
    ...
    if st.session_state.delete_info:
        ...
        st.divider()
        st.subheader(f"Supprimer : {ch.get('description')}")
        ...
        if check_last_in_category(ch.get("categorie"), data["charges"], ch.get("id")):
            st.warning(f"Dernière charge de la catégorie '{ch.get('categorie')}'. Elle sera vide après suppression.")
        
        d1, d2, _ = st.columns([1, 1, 2])
        with d1:
            if st.button("Confirmer la suppression", type="primary", key="confirm_del"):
    ...
    for cat in st.session_state.categories:
        ...
        with st.expander(f"{cat} — {fmt(cat_total)}", expanded=True):
            ...
            for ch in charges_cat:
                ...
                with c5:
                    sc, dc = st.columns(2)
                    if sc.button("Save", key=f"s_{ch.get('id')}", help="Sauvegarder"):
                        save_charge_montant(ch.get("id"), mois_ch, ANNEE, nl, no, m.get("id"))
                        st.toast("Sauvegardé")
                        refresh()
                    if dc.button("Del", key=f"d_{ch.get('id')}", help="Supprimer"):
...
with tabs[3]:
    ...
    with col1:
        ...
        for ep in data["epargne"]:
            ...
            val = c2.number_input("Val", value=float(m.get("montant", 0)), step=50.0, key=f"ep_{ep.get('id')}", label_visibility="collapsed")
            if c3.button("Save", key=f"sep_{ep.get('id')}"):
                save_epargne_montant(ep.get("id"), mois_ep, ANNEE, val, m.get("id"))
                st.toast("Sauvegardé")
                refresh()
            if c4.button("Del", key=f"dep_{ep.get('id')}"):
    ...
    with col2:
        ...
        for ep in data["epargne"]:
            ...
            if c3.button("Save", key=f"sobj_{ep.get('id')}"):
